...
if (isset($_POST['submit'])) {
	...
	else
	{
		...
		$query = mysqli_query($link, "SELECT * FROM user WHERE passwd='$pass' AND usernm='$username'") or die(mysqli_error());
		...
		if ($rows == 1) {
			...
			mysqli_query("UPDATE user SET last_login = NOW() WHERE id_user = '$a[id_user]'");
