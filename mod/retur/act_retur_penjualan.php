...
<?php
	...
	if($mod == "returpenjualan" AND $act == "simpan")
	{
		...
		$resCekRetur = mysqli_query($link, "SELECT * FROM tb_detail_penjualan 
		...
		if (mysqli_num_rows($resCekRetur) > 0) {
			...
			if($_POST['qty'] <= $r['qty'])
			{
				$resSimpan = mysqli_query($link, "INSERT INTO tb_retur_penjualan(no_transaksi, 
															kode_barang, 
															qty, 
															harga, 
															disc, 
	...
	elseif ($mod == "returpenjualan" AND $act == "hapus") {
		mysqli_query($link, "DELETE FROM tb_retur_penjualan 
