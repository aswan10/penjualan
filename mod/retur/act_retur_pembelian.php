...
<?php
	...
	if($mod == "returpembelian" AND $act == "simpan")
	{
		...
		$resCekRetur = mysqli_query($link, "SELECT * FROM tb_detail_pembelian 
		...
		if (mysqli_num_rows($resCekRetur) > 0) {
			...
			if($_POST['qty'] <= $r['qty'])
			{
				$resSimpan = mysqli_query($link, "INSERT INTO tb_retur_pembelian(no_faktur, 
															kode_barang, 
															harga_beli, 
															qty, 
															petugas, 
	...
	elseif ($mod == "returpembelian" AND $act == "hapus") {
		mysqli_query($link, "DELETE FROM tb_retur_pembelian 
