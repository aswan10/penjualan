	...
	switch ($act) {
		case 'form':
			if(!empty($_GET['id']))
			{
				...
				$query = mysqli_query($link, "SELECT * FROM tb_barang WHERE kode_barang = '$_GET[id]'");
					...
					<tr>
						...
						<td><div class='w3-row'>
							<div class='w3-col s4'>
								...
								$sql_kategori = mysqli_query($link, "SELECT * FROM tb_kategori_barang");
					...
					<tr>
						...
						<td><div class='w3-row'>
							<div class='w3-col s4'>
								...
								$sqlsatuan = mysqli_query($link, "SELECT * FROM tb_barang_satuan");
								while($s = mysqli_fetch_assoc($sqlsatuan))
								{
		...
		default :
						...
						<form class='w3-tiny' action='' method='GET'>	
							...
							<div class='w3-row'>
								...
								<div class='w3-col s3'>
									...
									$sqlkat = mysqli_query($link, "SELECT * FROM tb_kategori_barang");
									while ($k = mysqli_fetch_assoc($sqlkat)) {
										echo"<option value='$k[kategori_id]'>$k[nama_kategori]</option>";
									}
			...
			<table class='w3-table w3-striped w3-bordered w3-tiny w3-hoverable tbl'>
				...
				$sql_kul = mysqli_query($link, $query);
				...
				if($fd_kul > 0)
				{
					...
					$jmldata = mysqli_num_rows(mysqli_query($link, $q));
